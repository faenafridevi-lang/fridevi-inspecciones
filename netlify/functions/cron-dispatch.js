...
webpush.setVapidDetails('mailto:tu@mail.com', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);

export default async () => {
  // 1) suscripciones
  const subsList = await blobs.list({ prefix: 'subs/' });
  const subs = await Promise.all(subsList.blobs.map(b => blobs.get(b.key).then(r=>r.text()).then(JSON.parse)));

  // 2) recordatorios dentro de la ventana ±2h
  const remList = await blobs.list({ prefix: 'reminders/' });
  const reminders = await Promise.all(remList.blobs.map(b => blobs.get(b.key).then(r=>r.text()).then(JSON.parse)));
  ...
  return new Response(JSON.stringify({ ok:true, sent: results.length }), { status:200 });
};
